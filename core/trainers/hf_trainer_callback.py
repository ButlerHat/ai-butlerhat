...
from core.models.tokenization import AlfredTokenizer
...
class AlfredWandbCallback(WandbCallback):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        logger.info("AlfredWandbCallback initialized")
        self.table = self._wandb.Table(columns=["epoch", "prompt", "image", "label", "prediction"])

    def _create_rows(self, num_examples: int, epoch: str, dataloader: DataLoader, model: PreTrainedModel, tokenizer: AlfredTokenizer, args: CustomTrainingArguments) -> list:
    ...
    def on_evaluate(self, args: CustomTrainingArguments, state, control, **kwargs):
        logger.info("AlfredWandbCallback on_evaluate")
    ...
    def on_train_end(self, args, state, control, model=None, tokenizer=None, **kwargs):
        logger.info("AlfredWandbCallback on_train_end")
        super().on_train_end(args, state, control, model=model, tokenizer=tokenizer, **kwargs)
        self._wandb.log({"Alfred table": self.table})
