...
# Created another class AlfredTokenizer extending it to add special visual tokens like <loc_{}>, etc.

class AlfredTokenizer(T5Tokenizer):
...
class AlfredConverter(SpmConverter):
...
def convert_slow_alfredtokenizer(AlfredTokenizer):
    return AlfredConverter(AlfredTokenizer).converted()


class AlfredTokenizerFast(T5TokenizerFast):

    slow_tokenizer_class = AlfredTokenizer
    ...
    def __init__(
        ...
        **kwargs
        ...
        fast_tokenizer = convert_slow_alfredtokenizer(slow_tokenizer)
